...
void v_add_optimized_chunks(double* x, double* y, double* z) {
    // Do NOT use the `for` directive here!
    int num_threads, size_chunks;
    #pragma omp parallel
    {
        num_threads = omp_get_num_threads();
        size_chunks = ARRAY_SIZE / num_threads;
        int thread_id = omp_get_thread_num();
        int start = thread_id * size_chunks;
        int end = thread_id * size_chunks + size_chunks;
        for(int i = start; i < end; i++)
            z[i] = x[i] + y[i];
    }
    // tail case
    int i = (num_threads - 1) * size_chunks + size_chunks;
    for (; i < ARRAY_SIZE; i++) {
        z[i] = x[i] + y[i];
    }
}
